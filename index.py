...
def recommend(happy):
    ...
    name_place_admin = Standard1(place_admin["name"])

    if "user_id" in session:
        all_contribution = Contribution.objects(
            approved=True, happy=happy, user_id__ne=session["user_id"])
    else:
    ...
    if (len(all_contribution) > 0):
        ...
        user_place = Standard1(user_contribution.place["name"])
...
def contribute():
    ...
    if request.method == "GET":
        ...
        if form['link_img_song'] == "":
            f_img_song = "https://via.placeholder.com/150"
        if form['link_img_book'] == "":
            f_img_book = "https://via.placeholder.com/150"
        if form['link_img_video'] == "":
            f_img_video = "https://via.placeholder.com/160x90"
        new_song = Song(happy=form["happy_level"],
                        name=Standard(form["name_song"]),
                        author=Standard(form["author_song"]),
                        link=form["link_song"],
                        link_img=f_img_song,
                        user_id=session["user_id"])
        new_song.save()
        new_book = Book(happy=form["happy_level"],
                        name=Standard(form["name_book"]),
                        author=Standard(form["author_book"]),
                        link=form["link_book"],
                        link_img=f_img_book,
                        user_id=session["user_id"])
        new_book.save()
        new_video = Video(happy=form["happy_level"],
                          name=Standard(form["name_video"]),
                          author=Standard(form["author_video"]),
                          link=form["link_video"],
                          link_img=f_img_video,
                          user_id=session["user_id"])
        new_video.save()
        new_place = Place(happy=form["happy_level"],
                          name=Standard(form["name_place"]),
